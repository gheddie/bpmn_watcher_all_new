...
//		this.updateThread.start();
	...
	private void fillDeployedDefinitions() {		
		...
		try {
			...
			tbDeployedProcessDefinitions.setData(definitions, TableColumnConfigFactory.getDeployedDefinitionsColumn());
	...
	private void fillRunningInstances() {		
		...
		try {
			runningInstances = ProcessingSingleton.getInstance().queryInstances();
			tbRunningInstances.setData(runningInstances, TableColumnConfigFactory.getRunningInstancesColumnConfig());
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	}
	...
	private void putListeners() {
		...
		btnAddVariable.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				VariablesDialog dialog = new VariablesDialog(EnhandedProcessFrame.this, ((int) location.getX()) + 100, ((int) location.getY()) + 100);
				VariableInstanceDTO stagedVariable = dialog.getVariable();
				System.out.println("variable created : "+stagedVariable+".");
				stagedProcessVariables.put(stagedVariable.getName(), stagedVariable.getValue());
				fillProcessVariables();
			}
		});
		//---
	...
	private void fillProcessVariables() {
		...
		try {
			//deployed variables
			variableInstances = ProcessingSingleton.getInstance().queryVariables(selectedProcessInstance.getId());
			System.out.println(variableInstances.size() + " variable instances found.");
			
			//staged variables
			for (String key : stagedProcessVariables.keySet()) {
				variableInstances.add(new VariableInstanceDTO(key, stagedProcessVariables.get(key), VariableState.STAGED));
			}
			
			tbVariables.setData(variableInstances, TableColumnConfigFactory.getVaribleInstanceColumnConfig());
		} catch (RemoteException e) {
			e.printStackTrace();
		}		
	}
	...
	private void startProcessInstance() {
		...
		try {
			ProcessingSingleton.getInstance().startProcessInstanceByKey(selectedProcessDefinition.getKey(), stagedProcessVariables);
			initProcessVariables();
	...
	private void completeTask() {
		...
		try {
			logger.trace("trying to complete task '"+actualTask.getName()+"' [ID="+actualTask.getId()+"]...");
			ProcessingSingleton.getInstance().completeTask(actualTask.getId(), stagedProcessVariables);
			this.actualTask = null;
			refreshProcessPanel();
			initProcessVariables();
			resetError();
		} catch (BpmnException e) {
			lblError.setText(e.getMessage() + " ["+e.getCause().getClass().getCanonicalName()+"]");
		} catch (RemoteException e) {
			e.printStackTrace();
		}	
	}
	...
	public void refreshProcessPanel() {
		...
		try {
			activeTasks = ProcessingSingleton.getInstance().queryTasks(selectedProcessInstance.getId());
			bpmnPanel.initProcess(selectedProcessInstance);
			logger.trace(activeTasks.size() + " tasks set...");
			tbActualTasks.setData(activeTasks, TableColumnConfigFactory.getActualTasksColumnConfig());
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	}
	
	public void deployedDefinitionsBeanSelected(Object selectedProcessDefinitionObj) {
	...
	private void initComponents() {
		...
		//======== tbpMain ========
		{
			...
			//======== pnlOverview ========
			{
				...
				((GridBagLayout)pnlOverview.getLayout()).columnWidths = new int[] {624, 91, 0, 0};
				((GridBagLayout)pnlOverview.getLayout()).rowHeights = new int[] {297, 0, 114, 0};
				((GridBagLayout)pnlOverview.getLayout()).columnWeights = new double[] {1.0, 0.0, 0.0, 1.0E-4};
				((GridBagLayout)pnlOverview.getLayout()).rowWeights = new double[] {1.0, 0.0, 0.0, 1.0E-4};

				//======== bpmnView ========
				{
					bpmnView.setBorder(new TitledBorder("Aktueller Prozess-Status"));
					bpmnView.setLayout(new GridBagLayout());
					((GridBagLayout)bpmnView.getLayout()).columnWidths = new int[] {619, 0};
					((GridBagLayout)bpmnView.getLayout()).rowHeights = new int[] {292, 0};
					((GridBagLayout)bpmnView.getLayout()).columnWeights = new double[] {1.0, 1.0E-4};
					((GridBagLayout)bpmnView.getLayout()).rowWeights = new double[] {1.0, 1.0E-4};

					//======== bpmnScroller ========
					{
						bpmnScroller.setViewportView(bpmnPanel);
					}
					bpmnView.add(bpmnScroller, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 0), 0, 0));
				}
				pnlOverview.add(bpmnView, new GridBagConstraints(0, 0, 3, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 5, 0), 0, 0));

				//---- lblError ----
				lblError.setForeground(Color.red);
				pnlOverview.add(lblError, new GridBagConstraints(0, 1, 2, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 5, 5), 0, 0));

				//======== pnlActualTasks ========
				{
					pnlActualTasks.setBorder(new TitledBorder("Aktuelle Aufgaben"));
					pnlActualTasks.setLayout(new GridBagLayout());
					((GridBagLayout)pnlActualTasks.getLayout()).columnWidths = new int[] {475, 0};
					((GridBagLayout)pnlActualTasks.getLayout()).rowHeights = new int[] {76, 0, 0};
					((GridBagLayout)pnlActualTasks.getLayout()).columnWeights = new double[] {1.0, 1.0E-4};
					((GridBagLayout)pnlActualTasks.getLayout()).rowWeights = new double[] {1.0, 0.0, 1.0E-4};

					//======== scActualTasks ========
					{
						scActualTasks.setViewportView(tbActualTasks);
					}
					pnlActualTasks.add(scActualTasks, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 5, 0), 0, 0));

					//---- btnCompleteTask ----
					btnCompleteTask.setText("Aufgabe vollenden");
					pnlActualTasks.add(btnCompleteTask, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 0), 0, 0));
				}
				pnlOverview.add(pnlActualTasks, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 0, 5), 0, 0));

				//======== pnlRunningInstances ========
				{
					pnlRunningInstances.setBorder(new TitledBorder("Laufende Instanzen"));
					pnlRunningInstances.setLayout(new GridBagLayout());
					((GridBagLayout)pnlRunningInstances.getLayout()).columnWidths = new int[] {224, 0};
					((GridBagLayout)pnlRunningInstances.getLayout()).rowHeights = new int[] {119, 0, 0};
					((GridBagLayout)pnlRunningInstances.getLayout()).columnWeights = new double[] {0.0, 1.0E-4};
					((GridBagLayout)pnlRunningInstances.getLayout()).rowWeights = new double[] {0.0, 0.0, 1.0E-4};

					//======== scRunningInstances ========
					{
						scRunningInstances.setViewportView(tbRunningInstances);
					}
					pnlRunningInstances.add(scRunningInstances, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 5, 0), 0, 0));

					//---- btnRefreshInstances ----
					btnRefreshInstances.setText("Aktualisieren");
					pnlRunningInstances.add(btnRefreshInstances, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 0), 0, 0));
				}
				pnlOverview.add(pnlRunningInstances, new GridBagConstraints(1, 2, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 0, 5), 0, 0));

				//======== pnlVariables ========
				{
					pnlVariables.setBorder(new TitledBorder("Prozess-Variablen"));
					pnlVariables.setLayout(new GridBagLayout());
					((GridBagLayout)pnlVariables.getLayout()).columnWidths = new int[] {0, 0, 0, 0};
					((GridBagLayout)pnlVariables.getLayout()).rowHeights = new int[] {0, 0, 0};
					((GridBagLayout)pnlVariables.getLayout()).columnWeights = new double[] {0.0, 1.0, 0.0, 1.0E-4};
					((GridBagLayout)pnlVariables.getLayout()).rowWeights = new double[] {1.0, 0.0, 1.0E-4};

					//======== scVariables ========
					{
						scVariables.setViewportView(tbVariables);
					}
					pnlVariables.add(scVariables, new GridBagConstraints(0, 0, 3, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 5, 0), 0, 0));

					//---- btnRefreshVariables ----
					btnRefreshVariables.setText("Aktualisieren");
					pnlVariables.add(btnRefreshVariables, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 5), 0, 0));

					//---- btnAddVariable ----
					btnAddVariable.setText("+");
					pnlVariables.add(btnAddVariable, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 5), 0, 0));

					//---- btnRemoveVariable ----
					btnRemoveVariable.setText("-");
					pnlVariables.add(btnRemoveVariable, new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0,
						GridBagConstraints.CENTER, GridBagConstraints.BOTH,
						new Insets(0, 0, 0, 0), 0, 0));
				}
				pnlOverview.add(pnlVariables, new GridBagConstraints(2, 2, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 0, 0), 0, 0));
			}
			...
			//======== pnlDeployedDefinitions ========
			{
				...
				((GridBagLayout)pnlDeployedDefinitions.getLayout()).columnWidths = new int[] {0, 0, 0};
				((GridBagLayout)pnlDeployedDefinitions.getLayout()).rowHeights = new int[] {0, 0, 0};
				((GridBagLayout)pnlDeployedDefinitions.getLayout()).columnWeights = new double[] {1.0, 0.0, 1.0E-4};
				((GridBagLayout)pnlDeployedDefinitions.getLayout()).rowWeights = new double[] {1.0, 0.0, 1.0E-4};

				//======== scDeployedProcessDefinitions ========
				{
					scDeployedProcessDefinitions.setViewportView(tbDeployedProcessDefinitions);
				}
				pnlDeployedDefinitions.add(scDeployedProcessDefinitions, new GridBagConstraints(0, 0, 2, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 5, 0), 0, 0));

				//---- btnStartInstance ----
				btnStartInstance.setText("Instanz starten");
				pnlDeployedDefinitions.add(btnStartInstance, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 0, 5), 0, 0));

				//---- btnRefreshDefinitions ----
				btnRefreshDefinitions.setText("Aktualisieren");
				pnlDeployedDefinitions.add(btnRefreshDefinitions, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					new Insets(0, 0, 0, 0), 0, 0));
			}
